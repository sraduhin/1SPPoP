FILMWORKS = "SELECT id, modified " \
            "FROM content.film_work " \
            "ORDER BY modified;"

GENRES = "SELECT id, modified " \
         "FROM content.genre " \
         "ORDER BY modified;"
...
FILMWORKS_BY_P = "SELECT fw.id, fw.modified " \
...
FILMWORKS_BY_G = "SELECT fw.id, fw.modified " \
                 "FROM content.film_work fw " \
...
MISSING_DATA = "SELECT fw.id as fw_id, " \
               "fw.title, " \
               "fw.description, " \
               "fw.rating, " \
               "fw.type, " \
